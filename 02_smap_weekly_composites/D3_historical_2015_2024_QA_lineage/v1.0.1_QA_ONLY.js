/*
 * D3-SMAP-HIST-001 | Version 1.0.1 QA-only | 4 August 2026
 ...
 *
 * Persistence rule:
 *   - QA-only mode creates only three manifest tasks to Google Drive.
 *   - Annual Asset tasks remain disabled until the corrected QA manifest passes.
 *   - After QA approval, set createAssetTasks to true to create ten annual stacks
 *     that D4/D5/D9 will load again.
...
var CONFIG = {
  ...
  processingVersion: 'D3_SMAP_WEEKLY_1.0.1',
  parameterSignature: 'SMAP008_WIBMON_56_COV90_75_RANGEBYUNIT_OUTAGEFLAG_ANNUALSTACK_V101',
  driveFolder: 'Penelitian2026',
  assetRoot: 'projects/ee-adsgeophysics/assets',
  assetPrefix: 'D3_SMAP_WEEKLY_STACK',
  exportVersionTag: 'V101',
...
function weeklyComposite(weekStartWibInput, weekSequenceInput) {
  ...
  // Unit-aware range QA. Soil-moisture bands are volume fractions (0-0.9),
  // while sm_rootzone_pctl is a percentile product (0-100). Version 1.0.0
  // incorrectly applied 0-0.9 to the percentile band, masking valid pixels.
  var volumetricBands = scienceRaw.select([
    'sm_surface_mean',
    'sm_rootzone_mean',
    'sm_surface_min',
    'sm_rootzone_min',
  ...
  var volumetricRangeOk = volumetricBands.gte(0)
    .and(volumetricBands.lte(0.9))
    .reduce(ee.Reducer.min());
  var percentileBand = scienceRaw.select('sm_rootzone_pctl_native_mean');
  var percentileRangeOk = percentileBand.gte(0)
    .and(percentileBand.lte(100));
  var rangeOk = volumetricRangeOk
    .and(percentileRangeOk)
    .rename('range_ok');
...
function buildQaManifest(weeklyCollection) {
  return ee.FeatureCollection(weeklyCollection.toList(CONFIG.expectedWeeks).map(function(item) {
    ...
    var scienceFraction = ee.Number(
      scienceValidFraction.get('science_valid_fraction', 0)
    );
    ...
    var spatialQaFraction = scienceFraction
      .min(surfaceCoverageMean)
      .min(rootzoneCoverageMean);
    var imageCountQc = ee.String(image.get('global_qc_status'));
    var finalQc = ee.String(ee.Algorithms.If(
      spatialQaFraction.gte(CONFIG.passCoverage),
      imageCountQc,
      ee.Algorithms.If(
        spatialQaFraction.gte(CONFIG.warningCoverage),
        'PASS_WITH_WARNING',
        'HOLD'
      )
    ));
...
YEARS.forEach(function(year) {
  var annual = weekly
    ...
    .sort('system:time_start');
  var assetId = CONFIG.assetRoot + '/' + CONFIG.assetPrefix + '_' + year + '_' + CONFIG.exportVersionTag;
  ...
  if (CONFIG.createAssetTasks) {
    Export.image.toAsset({
      image: annualStack,
      description: CONFIG.assetPrefix + '_' + year + '_' + CONFIG.exportVersionTag,
      assetId: assetId,
      region: aoi,
...
print('D3 QA manifest rows prepared; expected 509', qaManifest.size());
print('D3 QA manifest export is the authoritative QA view; full Console preview disabled');
...
Export.table.toDrive({
  ...
  description: 'D3_SMAP_WEEKLY_QA_MANIFEST_2015_2024_V101',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'D3_SMAP_WEEKLY_QA_MANIFEST_2015_2024_V101',
  fileFormat: 'CSV'
});

Export.table.toDrive({
  collection: batchManifest,
  description: 'D3_SMAP_BATCH_MANIFEST_2015_2024_V101',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'D3_SMAP_BATCH_MANIFEST_2015_2024_V101',
  fileFormat: 'CSV'
});

Export.table.toDrive({
  collection: runManifest,
  description: 'D3_SMAP_RUN_MANIFEST_2015_2024_V101',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'D3_SMAP_RUN_MANIFEST_2015_2024_V101',
  fileFormat: 'CSV'
});