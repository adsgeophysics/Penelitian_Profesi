...
Version: V113 HYBRID PREDICTOR EXPORT — PROJECTION FIX
...
// -----------------------------------------------------------------------------
// V112 HYBRID PREDICTOR EXPORT
//
// FINAL A09 COMPUTE ARCHITECTURE:
//   GEE    -> predictor stacks only
//   Python -> frozen A08 Ridge inference + mosaicking + exact KHG mask
//
// Why:
// V111 still exceeded GEE worker memory even with:
//   - one orbit,
//   - simple rectangle,
//   - 1-km output,
//   - no complex KHG clipping.
// Therefore the Ridge inference graph is removed from GEE.
//
// This V112 test exports 3 predictor groups for each of the 3 orbit-120
// Sentinel-1 scenes in T05_NC during the WET pilot window.
// -----------------------------------------------------------------------------
...
var ORBIT = 120;
var BUFFER_M = 12000;
var EXPECTED_SCENES = 3;

var tileGeom = ee.Geometry.Rectangle(
  TILE.rect, 'EPSG:4326', false
);
var workGeom = tileGeom.buffer(BUFFER_M, 100);

var startWib = ee.Date(WINDOW.startWib);
var endWib = ee.Date(WINDOW.endExclusiveWib);
var startUtc = startWib.advance(-CONFIG.wibOffsetHours, 'hour');
var endUtc = endWib.advance(-CONFIG.wibOffsetHours, 'hour');

var scenes = s1
  .filterDate(startUtc, endUtc)
  .filterBounds(workGeom)
  .filter(ee.Filter.eq('relativeOrbitNumber_start', ORBIT))
  .sort('system:time_start');

var dailyGpm = buildDailyGpmCollection(
  startWib, endWib, workGeom
);

var GPM_FEATURE_NAMES = [
...
var S1_FEATURE_NAMES = [
  'vv_median_50m_db',
  'vh_median_50m_db',
  'vv_minus_vh_50m_db',
  'vh_vv_ratio_linear_50m',
  'rvi_50m',
  'angle_median_50m_deg',
  'days_since_prev_same_orbit_50m',
...
function scenePredictorGroups(im) {
  im = ee.Image(im);

  var gpmFeat = gpmAntecedentFeatures(
    im, dailyGpm, workGeom
  ).select(GPM_FEATURE_NAMES);

  var smapFeat = nearestSmap(
    im, workGeom
  ).select(SMAP_OUTPUT);

  // V113 PROJECTION FIX:
  // Some derived temporal bands originate from constants / mosaics and the
  // multi-band stack can therefore lose a single valid default projection.
  // reduceResolution() requires a valid default projection.
  //
  // Anchor the COMPLETE Sentinel-1 predictor stack to the CURRENT source
  // Sentinel-1 VV projection before aggregation. This does not resample the
  // values yet; it only supplies the correct default grid metadata.
  var s1NativeProj = im.select('VV').projection();

  var s1Feat = s1TemporalFeatures(
    im, workGeom
  )
  .select(S1_FEATURE_NAMES)
  .setDefaultProjection(s1NativeProj);

  // S1 is the fine-scale source. Aggregate the already-derived predictor
  // features to the 1-km scientific grid.
  var s1_1km = s1Feat
    .reduceResolution({
      reducer: ee.Reducer.mean(),
      bestEffort: true,
  ...
  // GPM and SMAP are intrinsically coarse. Keep their native hydrological
  // state and only place it on the common 1-km output grid (default nearest).
  var gpm_1km = gpmFeat
    .reproject({
      crs: CONFIG.exportCrs,
      scale: CONFIG.scale1km
...
// -----------------------------------------------------------------------------
// Scene QA — metadata only
// -----------------------------------------------------------------------------
var sceneQa = ee.FeatureCollection(
  scenes.toList(scenes.size()).map(function(obj) {
    var im = ee.Image(obj);
    ...
    return ee.Feature(null, {
      ...
      source_s1_crs: im.select('VV').projection().crs(),
      source_s1_nominal_scale_m: im.select('VV').projection().nominalScale(),
      tile_id: TILE.id,
      window_id: WINDOW.id,
...
Export.table.toDrive({
  collection: sceneQa,
  description: 'A09_V113_SCENE_QA',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'A09_V113_SCENE_QA',
  fileFormat: 'CSV'
});

// -----------------------------------------------------------------------------
// EXACTLY THREE scenes were confirmed by V111 QA.
// Create 3 groups x 3 scenes = 9 independent raster tasks.
// Run ONE task at a time.
// -----------------------------------------------------------------------------
var sceneList = scenes.toList(EXPECTED_SCENES);

for (var i = 0; i < EXPECTED_SCENES; i++) {
  var sceneNo = i + 1;
  var tag = 'SC0' + sceneNo;
  var im = ee.Image(sceneList.get(i));
  var groups = scenePredictorGroups(im);

  Export.image.toDrive({
    image: ee.Image(groups.s1).toFloat(),
    description: 'A09_V113_' + tag + '_S1_1KM',
    folder: CONFIG.driveFolder,
    fileNamePrefix: 'A09_V113_' + tag + '_S1_1KM',
    region: tileGeom,
    crs: CONFIG.exportCrs,
    scale: CONFIG.scale1km,
  ...
  Export.image.toDrive({
    image: ee.Image(groups.gpm).toFloat(),
    description: 'A09_V113_' + tag + '_GPM_1KM',
    folder: CONFIG.driveFolder,
    fileNamePrefix: 'A09_V113_' + tag + '_GPM_1KM',
    region: tileGeom,
    crs: CONFIG.exportCrs,
    scale: CONFIG.scale1km,
    maxPixels: CONFIG.maxPixels,
  ...
  Export.image.toDrive({
    image: ee.Image(groups.smap).toFloat(),
    description: 'A09_V113_' + tag + '_SMAP_1KM',
    folder: CONFIG.driveFolder,
    fileNamePrefix: 'A09_V113_' + tag + '_SMAP_1KM',
    region: tileGeom,
    crs: CONFIG.exportCrs,
    scale: CONFIG.scale1km,
    maxPixels: CONFIG.maxPixels,
...
print('A09 V113 HYBRID PREDICTOR EXPORT — PROJECTION FIX');
print('Scene count:', scenes.size());
print('Expected scene count from V111 QA:', EXPECTED_SCENES);
print('No Ridge inference is executed in GEE.');
print('Nine independent predictor tasks are available.');
print('RUN ONLY A09_V113_SC01_S1_1KM FIRST.');
print('If it completes, run SC01 GPM and SC01 SMAP.');
print('After SC01 is complete, upload the three rasters + scene QA.');
print('Final Ridge inference will be performed offline using the frozen A08 model.');
print('Exact KHG mask will be applied offline.');
print('SMAP peat-DTW predictor used?', false);

Map.centerObject(tileGeom, 8);
Map.addLayer(tileGeom, {color:'blue'}, 'V112 predictor-export tile');
