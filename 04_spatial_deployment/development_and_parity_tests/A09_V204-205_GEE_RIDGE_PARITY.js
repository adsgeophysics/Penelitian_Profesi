/*
A09 V204 — GEE-NATIVE CORRECTED-TIME FROZEN RIDGE PARITY
Scientific model: A08_FINAL_GWL_RIDGE_V102
Purpose: algebra/deployment optimization test only.
V203 offline raster remains the validated fallback architecture.
Run S02 and S03 station parity exports first. Do NOT start full-AOI production
until GEE-vs-joblib parity is PASS.
*/
...
var CONFIG = {
  ...
  outputVersion: 'A09_V204_GEE_RIDGE',
...
var SCENE_DEFS = [
  {tag:'S01', systemIndex:'S1A_IW_GRDH_1SDV_20190401T111558_20190401T111623_026595_02FB7A_1975', expectedOrbit:98, expectedCandidates:2, acquisitionWib:'2019-04-01 18:15:58'},
  {tag:'S02', systemIndex:'S1A_IW_GRDH_1SDV_20190402T224049_20190402T224114_026617_02FC54_6C84', expectedOrbit:120, expectedCandidates:3, acquisitionWib:'2019-04-03 05:40:49'},
  {tag:'S03', systemIndex:'S1A_IW_GRDH_1SDV_20190402T224114_20190402T224139_026617_02FC54_4BC9', expectedOrbit:120, expectedCandidates:4, acquisitionWib:'2019-04-03 05:41:14'},
  {tag:'S04', systemIndex:'S1A_IW_GRDH_1SDV_20190406T112359_20190406T112424_026668_02FE28_8922', expectedOrbit:171, expectedCandidates:6, acquisitionWib:'2019-04-06 18:23:59'},
  {tag:'S05', systemIndex:'S1A_IW_GRDH_1SDV_20190406T112424_20190406T112449_026668_02FE28_217E', expectedOrbit:171, expectedCandidates:2, acquisitionWib:'2019-04-06 18:24:24'},
  {tag:'S06', systemIndex:'S1A_IW_GRDH_1SDV_20190407T224839_20190407T224904_026690_02FEEB_BA34', expectedOrbit:18, expectedCandidates:1, acquisitionWib:'2019-04-08 05:48:39'}
];
...
function sampleOneScene(def) {
  ...
  return sampled.map(function(ft) {
    return ee.Feature(ft).set({
      ...
      acquisition_wib_corrected: t.advance(CONFIG.wibOffsetHours, 'hour')
...
SCENE_DEFS.forEach(function(def) {
  ...
  Export.table.toDrive({
    collection: table,
    description: 'A09_V204_GEE_RIDGE_' + def.tag,
    folder: CONFIG.driveFolder,
    fileNamePrefix: 'A09_V204_GEE_RIDGE_' + def.tag,
    fileFormat: 'CSV'
  });
});
...
Export.table.toDrive({
  collection: qa,
  description: 'A09_V204_GEE_RIDGE_QA',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'A09_V204_GEE_RIDGE_QA',
  fileFormat: 'CSV'
});
...
// ============================================================================
// A09 V205 — GEE-NATIVE RIDGE RASTER MEMORY TEST
// ============================================================================

var V205_TILE = ee.Geometry.Rectangle(
  [103.833333, -3.15, 105.166667, -1.5],
  'EPSG:4326',
  false
);

var V205_TARGET_SYSTEM_INDEX =
  'S1A_IW_GRDH_1SDV_20190402T224049_20190402T224114_026617_02FC54_6C84';

var v205Target = ee.Image(
  s1.filter(
    ee.Filter.eq('system:index', V205_TARGET_SYSTEM_INDEX)
  ).first()
);

var v205TargetUtc = ee.Date(v205Target.get('system:time_start'));
var v205TargetDayWib = dayStartWibFromS1(v205Target);

var v205WorkGeom = V205_TILE.buffer(12000, 100);

// Rainfall history required for antecedent features.
var v205DailyGpm = buildDailyGpmCollection(
  v205TargetDayWib.advance(-35, 'day'),
  v205TargetDayWib.advance(1, 'day'),
  v205WorkGeom
);

// Exact V204 predictor + frozen Ridge pipeline.
var v205PredStack = predictScene(
  v205Target,
  v205DailyGpm,
  v205WorkGeom
);

// 50-m MODEL SUPPORT raster.
var v205Gwl50 = v205PredStack
  .select('gwl_pred_m')
  .clip(V205_TILE)
  .rename('gwl_pred_m');

// PRIMARY scientific 1-km raster.
// Aggregation is performed AFTER Ridge inference.
var v205Gwl1k = v205Gwl50
  .reduceResolution({
    reducer: ee.Reducer.mean(),
    maxPixels: 1024,
    bestEffort: true
...
// Lightweight QA.
var v205Stats = v205Gwl1k.reduceRegion({
  reducer: ee.Reducer.minMax()
    .combine(ee.Reducer.mean(), '', true)
    .combine(ee.Reducer.percentile([1, 50, 99]), '', true),
  geometry: V205_TILE,
  scale: 1000,
  crs: CONFIG.exportCrs,
  maxPixels: 1e7,
...
print(
  'A09 V205 target UTC:',
  v205TargetUtc.format('YYYY-MM-dd HH:mm:ss', 'UTC')
);

print(
  'A09 V205 target WIB date:',
  v205TargetDayWib.format('YYYY-MM-dd')
);

print('A09 V205 1-km stats:', v205Stats);

// ============================================================================
// TASK 1 — RUN FIRST
// ============================================================================
Export.image.toDrive({
  image: v205Gwl50.toFloat(),
  description: 'A09_V205_S02_GWL_SUPPORT_50M_T05NC',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'A09_V205_S02_GWL_SUPPORT_50M_T05NC',
  region: V205_TILE,
  crs: CONFIG.exportCrs,
  scale: 50,
...
// ============================================================================
// TASK 2 — RUN ONLY AFTER TASK 1 COMPLETES
// ============================================================================
Export.image.toDrive({
  image: v205Gwl1k.toFloat(),
  description: 'A09_V205_S02_GWL_PRIMARY_1KM_T05NC',
  folder: CONFIG.driveFolder,
  fileNamePrefix: 'A09_V205_S02_GWL_PRIMARY_1KM_T05NC',
  region: V205_TILE,
  crs: CONFIG.exportCrs,
  scale: 1000,
  maxPixels: CONFIG.maxPixels,
...
print('A09 V205 RUN ORDER');
print('1. Run SUPPORT_50M first.');
print('2. If COMPLETE, run PRIMARY_1KM.');
print('3. If memory error, stop and return to V203 offline workflow.');